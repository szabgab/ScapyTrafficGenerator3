...
def main():
    ...
    parser.add_argument("--serverlan", help="server lan manager", required=False, default='Windows (TM) Code Name "Longhorn" ultimate 6.0')
    parser.add_argument("--clientlan", help="client lan manager", required=False, default='Samba 3.9.0-SVN-build-11572')
    parser.add_argument("--serveros", help="dns computer", required=False, default='Windows (TM) Code Name "Longhorn" ultimate 5231')
    parser.add_argument("--clientos", help="dns tree", required=False, default='Unix')
    ...
    parser.add_argument("-t", "--type", help="specify type of traffic", required=False, default='read')
    ...
    print 'set var'
    self.SMB.SetVariables(dmac=args.dstmac,
                           smac=args.srcmac,
                           TYPE=args.type,
                           src=args.src,
                           dst=args.dst,
                           ...
                           serverlanmanager=args.serverlan,
                           servernativeos=args.serveros,
                           clientnativeos=args.clientos,
                           clientLanManager=args.clientlan,
                           tree=args.treepath,
                           FILES=FILE,
                           )
    ...
    if args.type.lower() == 'read':
        print 'read'
        self.Packets = self.SMB.SMBTest(Read=True)
    elif args.type.lower() == 'write':
        self.Packets = self.SMB.SMBTest(Write=True)
    elif args.type.lower() == 'delete':
        self.Packets = self.SMB.SMBTest(Delete=True)
    elif args.type.lower() == 'serviceattack':
        self.Packets = self.SMB.SMBTest(ServiceAttack=True)
    else:
        print 'NO ACTION TAKEN'
        self.Packets = self.SMB.SMBTest()

    print '# packtes', len(self.Packets)
    if args.gretunnel:
        self.EncasulateGre()
    if args.vlanTag:
